...
2. `01_retained_model.ipynb` — the retained sparse logistic pipeline,
   date-grouped validation, calibration, stability and fitted contributions;
3. `02_qrt_baseline_vs_first_model.ipynb` — a paired comparison between the
   Ridge benchmark supplied by QRT and our first Ridge model, on identical
   folds and dates.
...
A complete execution requires the challenge CSV files in `data/`. End-to-end
recipes are defined in `src/pipelines.py`, preprocessing in
`src/preprocessing.py`, and date-grouped validation in
